#include "gz_CrazyflieInterface_plugin.h"
#include <future>
						...
						EventManager &_eventMgr)
{
	...
	m_queueSendCfFirm = moodycamel::BlockingConcurrentQueue<crtpPacket_t>(20);
	...
	receiverThread = std::thread(&GzCrazyflieInterface::recvThread, this);
	receiverCfLibThread = std::thread(&GzCrazyflieInterface::recvCfLibThread, this);
	senderThread = std::thread(&GzCrazyflieInterface::sendThread , this);
...
bool GzCrazyflieInterface::sendCf(const uint8_t* data , uint32_t length)
{	
...
void GzCrazyflieInterface::recvThread()
{
	uint8_t buf[32];
	while(isPluginOn)
	{
...
void GzCrazyflieInterface::recvCfLibThread()
{
	...
	while(isPluginOn)
	{
		...
		if (!socketInit_cfLib && buf[0] == 0xF3 && len == 1){
			gzdbg << "Received CfLib handshake message..." << std::endl;
...
void GzCrazyflieInterface::sendThread()
{
	crtpPacket_t msgs[10];
	while(isPluginOn){
		if(!socketInit)
			continue;
		size_t count = m_queueSendCfFirm.wait_dequeue_bulk(msgs, 10);
		for (size_t i = 0; i != count; i++) {
			sendCf(msgs[i].raw, msgs[i].size+1);
		}
	}
}
